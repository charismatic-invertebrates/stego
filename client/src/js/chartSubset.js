		...
		template = helpers.template = function(templateString, valuesObject){
			...
			function tmpl(str, data){
				...
				var fn = !/\W/.test(str) ?
					cache[str] = cache[str] :
					function(obj) {
					  var newStr = str.replace('<%=', '').replace('%>', '').trim();

					  if (data['datasetLabel'] !== undefined) {
              return data['datasetLabel'] + ': ' + data[newStr];
					  } else {
						  return data[newStr];
					  }
					};
				...
				if (this.datasets && this.datasets.length > 1) {
					...
					each(ChartElements, function(Element) {
						...
						new Chart.Tooltip({
							...
							cornerRadius: this.options.tooltipCornerRadius,
							text: template(this.options.multiTooltipTemplate, Element),
							chart: this.chart,
							custom: this.options.customTooltips
						}).draw();
					}, this);
